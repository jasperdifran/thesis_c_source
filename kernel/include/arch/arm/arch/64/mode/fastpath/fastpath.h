...
static inline void fastpath_copy_mrs(word_t length, tcb_t *src, tcb_t *dest)
{
    ...
    setRegister(dest, X2, getRegister(src, X2));
    setRegister(dest, X3, getRegister(src, X3));
    setRegister(dest, X4, getRegister(src, X4));
    setRegister(dest, X5, getRegister(src, X5));
...
static inline void NORETURN FORCE_INLINE fastpath_restore_other_fast_registers(word_t badge, word_t msgInfo, tcb_t *cur_thread, tcb_t *thread)
{
    NODE_UNLOCK;
...
// We know there is only 1 dom
static inline prio_t getHighestPrio_fp(void) {
    word_t l1index;
    word_t l2index;
    word_t l1index_inverted;

    l1index = wordBits - 1 - clzl(NODE_STATE(ksReadyQueuesL1Bitmap)[0]);
    l1index_inverted = invert_l1index(l1index);

    l2index = wordBits - 1 - clzl(NODE_STATE(ksReadyQueuesL2Bitmap)[0][l1index_inverted]);
    return (l1index_to_prio(l1index) | l2index);
}

static inline void activateThread_fp(tcb_t *thread) {
    ...
    case ThreadState_IdleThreadState:
        Arch_activateIdleThread(thread);
        break;
    default:
        break;
    }
...
static inline void FORCE_INLINE  choose_new_thread_fp(void) {
    ...
    if (likely(NODE_STATE(ksReadyQueuesL1Bitmap[0])))
    {
        prio = getHighestPrio_fp();
        thread = NODE_STATE(ksReadyQueues)[prio].head;
        newVTable = TCB_PTR_CTE_PTR(thread, tcbVTable)->cap;

        cap_pd = cap_vtable_cap_get_vspace_root_fp(newVTable);
        stored_hw_asid.words[0] = cap_vtable_root_get_mappedASID(newVTable);
        switchToThread_fp(thread, cap_pd, stored_hw_asid);
    }
    else
    {
        switchToIdleThread();
