...
#ifdef CONFIG_EXCEPTION_FASTPATH
static inline FORCE_INLINE void NORETURN fastpath_vm_fault(vm_fault_type_t type)
{
    cap_t handler_cap;
    endpoint_t *ep_ptr;
    tcb_t *dest;
    ...
    if (unlikely(!cap_capType_equals(handler_cap, cap_endpoint_cap)
                 ...
                 || !cap_endpoint_cap_get_capCanSend(handler_cap) || (!cap_endpoint_cap_get_capCanGrant(handler_cap) && !cap_endpoint_cap_get_capCanGrantReply(handler_cap))
#endif
                     ))
    {
...
static inline FORCE_INLINE void NORETURN fastpath_simple_call(word_t tcbDest, word_t msgInfo)
{
    seL4_MessageInfo_t info;
    tcb_t *dest;
    cap_t newVTable;
    ...
    NODE_STATE(ksCurThread)->tcbState.words[0] = ThreadState_BlockedOnReply;

    fastpath_copy_mrs(seL4_FastMessageRegisters, NODE_STATE(ksCurThread), dest);

    /* Dest thread is set Running, but not queued. */
    dest->tcbState.words[0] = ThreadState_Running;
    switchToThread_fp(dest, cap_pd, stored_hw_asid);
...
static inline FORCE_INLINE void NORETURN fastpath_simple_reply_recv(word_t destTCB, word_t msgInfo)
{
    seL4_MessageInfo_t info;
    tcb_t *dest;
    ...
    NODE_STATE(ksCurThread)->tcbState.words[0] = ThreadState_BlockedOnReceive;
...
fastpath_simple_recv(word_t destTCB, word_t msgInfo)
{
    tcb_t *curthread = NODE_STATE(ksCurThread);
    if (unlikely(destTCB == 0))
    {
        /* Only used to block a thread forever, we can ignore this case for longest DI path*/
        setThreadState(curthread, ThreadState_Inactive);
        tcbSchedDequeue(curthread);
        scheduleTCB(curthread);
        schedule();
        activateThread();
    }
    else
    {
        if (curthread->notifier == 0)
        {
            /* Idle notification */
            thread_state_ptr_set_tsType(&curthread->tcbState,
            ...
            /* Required for both. Used if we are switching to a thread in the 'restore' state. */
            activateThread();
        }
        else
        {
            /* Active notification */
            curthread->notifier = 0;
        }
    }
    restore_user_context();
}
...
fastpath_simple_send(word_t destTCB)
{
    tcb_t *curthread = NODE_STATE(ksCurThread);
    tcb_t *dest = (tcb_t *)destTCB;

    if (dest->tcbState.words[0] == ThreadState_Running)
    {
        dest->notifier = (word_t)curthread;
    }
    else
    {
        thread_state_ptr_set_tsType(&dest->tcbState,
                                    ThreadState_Running);
