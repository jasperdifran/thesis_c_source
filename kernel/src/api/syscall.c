...
exception_t handleUnknownSyscall(word_t w)
{
    ...
    case SysBenchmarkInvalidateTlb:
...
exception_t handleSyscall(syscall_t syscall)
{
    ...
    MCS_DO_IF_BUDGET({
        ...
        case SysSimpleGetTCBPointer:;
            ...
            setRegister(NODE_STATE(ksCurThread), capRegister, cap_thread_cap_get_capTCBPtr(lu_ret.cap));
            break;

        case SysSimpleTCBReset:;
            // cptr = getRegister(NODE_STATE(ksCurThread), capRegister);
            // lu_ret = lookupCapAndSlot(NODE_STATE(ksCurThread), cptr);
            // tcb_t *target = TCB_PTR(cap_thread_cap_get_capTCBPtr(lu_ret.cap));
