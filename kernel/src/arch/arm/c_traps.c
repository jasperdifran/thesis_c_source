...
void VISIBLE c_handle_fastpath_simple_call(word_t tcbDest, word_t msgInfo)
{
    NODE_LOCK_SYS;

    c_entry_hook();

    fastpath_simple_call(tcbDest, msgInfo);
    UNREACHABLE();
}

ALIGN(L1_CACHE_LINE_SIZE)
void VISIBLE c_handle_fastpath_simple_reply_recv(word_t tcbDest, word_t msgInfo)
{
    NODE_LOCK_SYS;

    c_entry_hook();

    fastpath_simple_reply_recv(tcbDest, msgInfo);
    UNREACHABLE();
}

ALIGN(L1_CACHE_LINE_SIZE)
void VISIBLE c_handle_fastpath_simple_recv(word_t tcbDest, word_t msgInfo)
{
    NODE_LOCK_SYS;

    c_entry_hook();

    fastpath_simple_recv(tcbDest, msgInfo);
    UNREACHABLE();
}

extern void *lower_el_sync;

ALIGN(L1_CACHE_LINE_SIZE)
void VISIBLE c_handle_fastpath_simple_send(word_t tcbDest, word_t msgInfo)
{
    NODE_LOCK_SYS;

    c_entry_hook();

    fastpath_simple_send(tcbDest, msgInfo);
    UNREACHABLE();
}

ALIGN(L1_CACHE_LINE_SIZE)
void VISIBLE c_handle_get_cur_time(void)
{
    NODE_LOCK_SYS;

    word_t time = timestamp();

    c_entry_hook();

    setRegister(NODE_STATE(ksCurThread), msgRegisters[0], time);

    setRegister(NODE_STATE(ksCurThread), msgRegisters[1], timestamp());
